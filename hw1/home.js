...
function modale(event){
    ...
    fetch("http://localhost/hw1/like_users.php?idPost=" + boxPost.id).then(onResponseLikeUsers).then(onJSONLikeUsers);
}
...
function aggiungiLike(event){
    ...
    fetch("http://localhost/hw1/metti_like.php?idPost=" + boxPost.id).then(onResponseAddLike).then(onJSONAddLike);
}
...
function onJSON(json)
{
    ...
    for(post of json){
        ...
        fetch("http://localhost/hw1/stampa_like.php?idPost=" + post.idPost).then(onResponseLike).then(onJSONLike);
...
fetch("http://localhost/hw1/utenti_seguiti.php").then(onResponse).then(onJSON);